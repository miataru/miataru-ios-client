...
`Current` means the document describes the development checkout and must be checked against source and Xcode settings. `P0 target` and `Later` are proposals. `historisch` is retained evidence, not a Current instruction. The app and widget share development metadata 3.5 build 3 in this checkout. Neither the source version nor an archived changelog entry proves the current App Store state.

## Review decisions

- The project overview, feature guide, development guide, root README, and App Store copy had stale 3.2-series headings. They now distinguish the 3.5 build 3 checkout from distribution status. The App Store copy is a draft; release-facing claims need validation during a requested release.
