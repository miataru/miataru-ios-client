...
| Location Update Pipeline (`LocationManager`, `DeviceLocationRefresher`, retry/outbox path) | Very High | Yes (21) | No | No | No | Medium-High | Medium | Add integration tests for GPS updates, network failures, retry/backoff, outbox flush triggers, and real background/reboot behavior on device. |
...
| P1 (done 2026-05-26) | Protect reboot-safe background tracking decisions | Done: recovery-anchor eligibility, launch recovery gating, location-launch detection, and duplicate location callback suppression are unit-tested | Expanded `SettingsConfigurationTests` with `LocationManager` and `AppDelegate` recovery decision coverage. |
