...
| App Intents, Siri, and Shortcuts (`TrackedPersonEntity`, `TrackedPersonQuery`, `FindPersonLocationIntent`, `OpenRouteToPersonIntent`, `IntentLocationService`) | Medium-High | Yes (7) | No | No | Manual plan only | Medium | Medium | Add manual device/Siri validation for App Shortcut discovery, Siri phrase recognition, and Apple Maps handoff; add integration tests once a fake server layer exists. |
...
| P1 (done 2026-06-09) | Add App Intents preparation coverage | Done: known-device entity mapping, current-location-access filtering, API-location mapping, no-location and unauthorized errors, Maps URL privacy, and App Intent localization completeness are unit-tested | Added `AppIntentsPreparationTests` and documented manual Siri/Shortcuts validation in `documentation/AppIntentsManualTestPlan.md`. |
