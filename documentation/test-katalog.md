...
| UT-SET-010 | Primary significant-change callback reasserts only active frequent runtime sessions | Protect frequent background cadence after primary recovery-anchor callbacks and stale secondary callbacks | A primary significant-change callback reasserts frequent background updates only when frequent standard updates were already active in the current process; cold reboot/location relaunch, foreground callbacks, mode switches, and secondary callbacks do not trigger it. The first stale frequent-background callback after disabling frequent mode may still upload a valid location, but repeated stale callbacks are suppressed after cleanup | LocationManager Core Location manager callback policy | Unit | Static app-state/source/mode-switch inputs | High |
