...
| UT-HPA-001 | Map pan or zoom starts or restarts auto-hide | History panel auto-hide source | Map pan/zoom event -> start/restart timer | HistoryPanelAutoHidePolicy | Unit | Static event policy | High |
| UT-HPA-002 | Map point selection starts or restarts auto-hide | History point selection source | Map point selection event -> start/restart timer | HistoryPanelAutoHidePolicy | Unit | Static event policy | High |
| UT-HPA-003 | Play or pause starts or restarts auto-hide | Playback exception | Play/pause event -> start/restart timer | HistoryPanelAutoHidePolicy | Unit | Static event policy | High |
| UT-HPA-004 | Non-playback panel interactions do not affect auto-hide | Panel interaction boundary | Speed change, scrub, range drag, quick range -> no timer action | HistoryPanelAutoHidePolicy | Unit | Static event policy | High |
| UT-HPA-005 | Programmatic and lifecycle events do not affect auto-hide | Programmatic camera guard | Programmatic camera/view lifecycle events -> no timer action | HistoryPanelAutoHidePolicy | Unit | Static event policy | High |
...
| UT-AIP-001 | KnownDevice maps to tracked device entity | Protect App Entity mapping | `KnownDevice` display name, DeviceID, and current-location access map to `IntentDeviceRecord` and `TrackedDeviceEntity` | App Intents / IntentLocationService | Unit | KnownDevice fixture | High |
| UT-AIP-002 | Suggested devices only include visible devices with current location access | Enforce Siri/Shortcuts visibility boundary | Blank IDs and devices without current-location access are filtered out | App Intents / TrackedDeviceQuery source | Unit | Fake intent provider | High |
| UT-AIP-003 | API location payload maps to intent device location | Protect location DTO mapping | `MiataruLocationData` maps to `IntentLocationPayload` and `IntentDeviceLocation` with cached place text | App Intents / IntentLocationService | Unit | MiataruLocationData fixture | High |
| UT-AIP-004 | Latest location throws when no location exists | Validate no-location error | Visible device with no payload throws `.noLocationAvailable` | App Intents / IntentLocationService | Unit | Fake intent provider | High |
| UT-AIP-005 | Device without current location access is not visible and cannot be queried | Validate authorization privacy boundary | Hidden device is not suggested, cannot resolve as entity, and throws `.unauthorized` on stale shortcut execution | App Intents / IntentLocationService | Unit | Fake intent provider | High |
| UT-AIP-006 | Apple Maps URL uses coordinates and does not leak device identifiers | Prevent DeviceID/name leakage in route action | Maps URL contains only destination coordinates and no device ID/name | App Intents / OpenRouteToPersonIntent | Unit | IntentDeviceLocation fixture | High |
| UT-AIP-007 | App Intent localization keys exist for all app locales | Enforce App Intent localization completeness | Every `intent_` key in `AppIntents.xcstrings`, known AppIntents-extracted summary fallback key, and App Shortcut phrase set has a non-empty value for all 10 app locales | App Intents / split string-catalog QA | Unit | JSON parse of `Assets/Localization/*.xcstrings` | High |
| UT-AIP-008 | Miataru navigation URL uses device ID and focused user-to-device mode | Protect in-app navigation handoff | The Miataru navigation intent creates a `miataru://` link with DeviceID, `action=navigate`, `direction=userToDevice`, `presentation=focused`, and no display-name leakage | App Intents / OpenMiataruNavigationToPersonIntent | Unit | IntentDeviceLocation fixture | High |
...
| UT-AIP-010 | Place entity maps and indexed attributes exclude private location data | Protect saved-place entity privacy | `MiataruPlaceRecord` maps to `MiataruPlaceEntity`; Spotlight attributes include name/radius and omit latitude/longitude | App Intents / MiataruPlaceEntity | Unit | Saved-place fixture | High |
| UT-AIP-011 | Place user activity annotation is searchable without raw coordinates | Protect future view annotation behavior | iOS 26 user activity annotation attaches the place entity without raw coordinate userInfo | App Intents / MiataruPlaceIntentMetadata | Unit | `NSUserActivity`, availability guard | Medium |
| UT-AIP-012 | Place query resolves exact IDs and case-insensitive names | Protect AppEntity lookup and search | `MiataruPlaceQuery` resolves UUIDs and filters names case-insensitively | App Intents / MiataruPlaceQuery | Unit | Fake intent service | High |
| UT-AIP-013 | Saving current place uses current location and rejects stale locations | Protect per-device current-location save semantics | Current user location creates a place for the selected device; stale locations throw `.currentLocationTooOld` | App Intents / IntentLocationService + MiataruPlaceStore | Unit | Fake current location, temp place store | High |
...
| UT-PLS-004 | Corrupt place file starts empty and removes invalid file | Protect corrupt-store recovery | Invalid JSON is deleted and store starts empty | MiataruPlaceStore | Unit | Temp corrupt JSON file | High |
...
| UT-OUT-005 | Visitor check context is persisted with queued updates | Preserve known-visitor trigger context | Queued update reload keeps visitor minimum interval and `processKnownVisitorAlerts` flag | LocationUpdateOutboxStore | Unit | temp outbox file + Codable roundtrip | High |
...
| UT-LDC-011 | Flush forwards queued visitor check context | Preserve frequent visitor-check metadata | Successful flush forwards visitor minimum interval and known-visitor processing flag to the visitor processor | LocationUpdateDeliveryCoordinator | Unit | mock sender + recording visitor processor | High |
| UT-LDC-012 | Submit during in-flight direct send queues without waiting | Prevent missing updates behind a stuck direct send | Later submissions append to the outbox while the first direct upload remains blocked | LocationUpdateDeliveryCoordinator | Unit | blocking sender actor + temp outbox | High |
| UT-LDC-013 | In-flight direct send drains queued tail in FIFO order | Preserve ordering after direct send completion | Releasing the first direct upload flushes the queued tail in original order | LocationUpdateDeliveryCoordinator | Unit | blocking sender actor + deferred drain | High |
| UT-LDC-014 | Retryable in-flight direct failure queues original before later submissions | Preserve FIFO on transient direct failure | Retryable first-send failure prepends the original payload ahead of updates queued while it was in flight | LocationUpdateDeliveryCoordinator | Unit | blocking failure sender + temp outbox | High |
| UT-LDC-015 | Server URL retarget replays in-flight direct send to new URL | Protect retarget race for direct sends | Retargeting during a blocked direct upload schedules a replay to the new server URL after the old request resolves | LocationUpdateDeliveryCoordinator | Unit | blocking sender actor + retarget during direct send | High |
...
| UT-UVA-007 | Unknown visitor alert localization keys exist for all app locales | Localization completeness gate | Required keys present and non-empty for all 10 locales across the themed app catalogs | Split string-catalog QA | Unit | JSON parse of `Assets/Localization/*.xcstrings` | High |
| UT-UVA-008 | Watched known visitor is selected from recent VisitorHistory | Validate known-device candidate selection | Watched known device with newest timestamp inside 90s window produces a known visitor candidate | KnownVisitorAlertEvaluator | Unit | MiataruVisitor fixture + fixed timestamp | High |
| UT-UVA-009 | Unwatched and own devices do not trigger known visitor alerts | Protect opt-in and privacy boundaries | Unwatched known devices and the current device are excluded before notification scheduling | KnownVisitorAlertEvaluator | Unit | watched-device fixtures | High |
| UT-UVA-010 | Known visitor 90-second window matches recent visitor indicator | Keep notification window aligned with eye indicator | Visitor older than the 90s recent window is ignored | KnownVisitorAlertEvaluator | Unit | fixed clock | High |
| UT-UVA-011 | Known visitor duplicate timestamp is suppressed | Prevent repeated notification for same visit | Previously notified timestamp for same device produces no candidate | KnownVisitorAlertEvaluator | Unit | persisted timestamp fixture | High |
| UT-UVA-012 | Known visitor cooldown values are respected | Validate configurable cooldown | Candidate is suppressed inside cooldown and allowed once cooldown expires | KnownVisitorAlertEvaluator | Unit | fixed clock + cooldown values | High |
| UT-UVA-013 | Watched known visitor fetch runs without unknown alerts | Validate independent known-device feature path | Unknown alerts off, watched known device on, explicit frequent context fetches VisitorHistory and schedules known notification | UnknownVisitorAlertService | Unit | actor notifier/data-provider mocks | High |
| UT-UVA-014 | Known visitor alerts are skipped outside frequent visitor checks | Prevent foreground/open false positives | Smart Frequent setting can be on, but without explicit frequent visitor-check context no known notification is scheduled | UnknownVisitorAlertService | Unit | actor notifier/data-provider mocks | High |
| UT-UVA-015 | Unknown and known visitor alerts share one VisitorHistory fetch | Prevent duplicate network work | Unknown alert and watched known alert processing run from one shared VisitorHistory response | UnknownVisitorAlertService | Unit | actor notifier/data-provider mocks | High |
...
| UT-SET-002 | Settings localization keys exist for all app locales | Enforce localization completeness | Required settings labels/explanations are present and non-empty for all 10 locales across the themed app catalogs | Split string-catalog QA | Unit | JSON parse of `Assets/Localization/*.xcstrings` | High |
...
| UT-SET-005 | Known visitor cooldown defaults and normalization are stable | Protect advanced cooldown setting | Default 30m, allowed 1/5/15/30/60m values, invalid fallback, app-activation refresh, and persistence are covered | SettingsManager / SettingsConfiguration | Unit | isolated `UserDefaults` suite | High |
| UT-SET-006 | KnownDevice visitor notification flag roundtrips and legacy archives default off | Protect per-device opt-in persistence | Secure archive preserves true value; decoder without the new key defaults to false | KnownDevice | Unit | NSSecureCoding roundtrip + legacy decoder | High |
...
| UT-LTP-011 | Frequent visitor checks require background effective frequent runtime | Prevent foreground known-visitor alerts | Visitor check eligibility is true only outside active app state while effective frequent updates are running | LocationTrackingPolicy | Unit | Static app-state/frequent inputs | High |
...
| UT-SFB-004 | Smart frequent exit fence eligibility and radius are configurable | Protect hidden exit-fence policy | Eligibility and configurable radius calculation honor tracking/auth/manual/runtime constraints | SmartFrequentBackgroundPolicy | Unit | Static policy inputs | High |
...
| UT-SET-017 | App string catalog has no stale or new translation units | Keep localized release catalog clean | All app `.xcstrings` catalogs under `Assets/Localization/` contain no `extractionState == stale` entries and no new `stringUnit` or `stringSet` states | Split string-catalog QA | Unit | JSON parse of `Assets/Localization/*.xcstrings` | High |
