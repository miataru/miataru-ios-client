...
- Added an enabled-by-default location tracking health reminder that schedules one local notification after five days without app foreground activity or a successfully sent own-location update, with configurable 5/7/14/30-day intervals in Advanced Options and iOS Settings.
- The health reminder now requests notification permission only while location tracking is enabled, cancels pending and delivered reminders when tracking is disabled or permission is denied, and leaves notification taps on the default app-open behavior.
- Added Intent Sprint Step 05 as a device-scoped Places foundation with an Application Support JSON store, `MiataruPlaceEntity`, name search, and App Intents for saving the current place, listing places, checking one device near one of its places, and finding visible devices near a saved place.
- Kept Places ready for a future device-detail UI by requiring every place to own a tracked DeviceID, rejecting duplicate place names only within the owning device, allowing the same place name across devices, and keeping raw coordinates/device IDs out of spoken dialogs and JSON output.
- Added localized place/proximity intent titles, parameters, summaries, dialogs, and errors across all supported app locales while removing stale string-catalog entries for the previous global-place summaries.
- Added focused Places coverage for device-scoped persistence, corrupt-file recovery, validation, same-name-across-devices behavior, entity/query privacy, stale-current-location rejection, proximity accuracy allowance, hidden-device exclusion, and output privacy.
