...
- `LocationManager` is intentionally a facade: tracking decisions are implemented in `LocationTrackingPolicy`, Smart runtime behavior in `SmartFrequentBackgroundPolicy`, upload/cache sample plausibility and deduplication in `LocationSamplePolicy`, and background diagnostic decisions in `LocationBackgroundForensics`.
...
- Upload payload/background-task handling, 24-hour location-update metrics, and heading/course smoothing are encapsulated in `LocationUpdateUploadService`, `LocationUpdateMetricsStore`, and `HeadingSmoother`.
- `LocationSamplePolicy` rejects invalid/coarse fixes and short implausible speed spikes before cache/upload, and defers suspicious 500 m+ jumps after long callback gaps until a nearby follow-up fix confirms the new position. Raw diagnostics and Smart/Frequent recovery can still inspect the original samples.
